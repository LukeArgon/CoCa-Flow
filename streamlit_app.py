...
if 'user' not in st.session_state:
    ...
    with st.form("login"):
        ...
        if submit and nome:
            user_data = {
                ...
                # Inizializziamo un campo appunti vuoto nel DB se non esiste
                "appunti_temp": "" 
            }
            # Salva/Aggiorna utente su Firebase
            # Usiamo merge=True per non cancellare eventuali appunti salvati in precedenza se l'utente rientra
            db.collection("partecipanti").document(nome).set(user_data, merge=True)
            
            # Recuperiamo subito gli appunti se c'erano
    ...
    # Aggiorniamo i dati utente dal DB per essere sicuri di avere gli ultimi appunti
    ...
    with st.sidebar:
        ...
        if st.button("Disconnetti", type="primary"):
            # Non cancelliamo l'utente dal DB, lo segniamo solo offline se volessimo (qui lo rimuoviamo dalla sessione locale)
            # Se vuoi rimuoverlo dalla lista visiva:
    ...
    # ---------------------------------------------------------
    with tab1:
        # Layout: Colonna Sinistra (Discussione), Colonna Destra (Appunti Personali)
        ...
        # COLONNA APPUNTI (Visibile sia a Admin che Partecipanti)
        with col_notes:
            st.subheader("I tuoi Appunti")
            st.caption("Scrivi qui le tue note o idee. Vengono salvate automaticamente.")
            
            # Leggiamo gli appunti attuali dalla sessione o dal DB
        ...
        with col_prop:
            ...
            for p in proposti:
                ...
                with st.expander(f"{p_data['titolo']} ({p_data['proponente']})"):
                    ...
                    # VISUALIZZAZIONE ETICHETTE SPUNTATE
                    lbls = []
    ...
    with tab3:
        ...
        for u in users:
            ...
            with col_a:
                st.write(f"**{u_data['nome']}**")
            with col_b:
                st.write(f"{u_data['ruolo']}")
            with col_c:
    ...
    with tab4:
        ...
        # 1. EXPORT EXCEL (Due Fogli)
        if 'h_list' in locals() and h_list:
            # Prepariamo i dati per il foglio Cronologia
            data_topics = []
            for x in h_list:
                row = {k:v for k,v in x.items() if k != '_dt'}
                data_topics.append(row)
            df_topics = pd.DataFrame(data_topics)

            # Prepariamo i dati per il foglio Presenti
            # Rileggiamo i presenti attuali
            pres_now = [u.to_dict() for u in db.collection("partecipanti").stream()]
            df_users = pd.DataFrame(pres_now)
            ...
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                ...
                if not df_users.empty:
                    ...
                    # Crea foglio vuoto se non ci sono utenti (raro)
                    pd.DataFrame(["Nessuno"]).to_excel(writer, sheet_name='Presenti')
            
            st.download_button(
                label="Scarica Report Excel (.xlsx)",
                data=buffer.getvalue(),
                file_name=f"Report_Riunione_{current_title}.xlsx",
        ...
        # 3. VISUALIZZAZIONE ARCHIVI (Con elenco topic espanso)
